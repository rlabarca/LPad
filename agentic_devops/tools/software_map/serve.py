...
class GraphHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/api/graph':
            ...
            graph_type = query.get('type', ['app'])[0]
            mmd_file = MMD_FILE_AGENTIC if graph_type == 'agentic' else MMD_FILE_APP
            ...
            possible_paths = [
                os.path.join(FEATURES_DIR_APP, feature_name),
                os.path.join(FEATURES_DIR_APP, f"{feature_name}.md"),
