/**
 * @file demo_v058_entry.cpp
 * @brief Release 0.58 Demo Entry Point Implementation
 */
...
// Global instances
static V058DemoApp* g_demoApp = nullptr;
...
void demo_setup() {
    ...
    Serial.println("\n\n\n=== LPad Release 0.58 Demo Application ===");
    Serial.println("Composite Demo Pattern: V058DemoApp");
    Serial.flush();
    yield();
    ...
    // [4/4] Create V058DemoApp
    Serial.println("[4/4] Creating V058DemoApp...");
    Serial.flush();

    static V058DemoApp demoApp;
    g_demoApp = &demoApp;

    if (!g_demoApp->begin(g_relativeDisplay)) {
        displayError("V058DemoApp initialization failed");
        while (1) delay(1000);
    }

    Serial.println("  [PASS] V058DemoApp initialized");
    Serial.println();

    Serial.println("=== Release 0.58 Demo Application Ready ===");
    Serial.println();
    Serial.println("Demo Cycle (repeats indefinitely):");
    Serial.println("  1. Connectivity Check (Wi-Fi + Ping)");
    Serial.println("  2. Hold 'PING OK' for 2 seconds");
    Serial.println("  3. V0.58 Dynamic Visual Demo:");
    Serial.println("     - Logo Animation (wait 2s + animate 1.5s + hold 2s)");
    Serial.println("     - Graph with LIVE DATA (new point every 1 second)");
    Serial.println("     - 6 visual modes cycling every 5 seconds");
    Serial.println("  4. Return to step 1");
    Serial.println();
