/**
 * @file main.cpp
 * @brief LPad Main Application Entry Point - Release 0.55 Demo
 *
 * Implements the composite demo pattern using V055DemoApp.
 * See features/demo_release_0.55.md for specification.
 */

#include <Arduino.h>
#include <Arduino_GFX_Library.h>
#include "../demos/v055_demo_app.h"
#include "display.h"
#include "relative_display.h"
#include "animation_ticker.h"

// Global instances
V055DemoApp* g_demoApp = nullptr;
...
void setup() {
    ...
    Serial.println("Composite Demo Pattern: V055DemoApp");
    Serial.flush();
    yield();
    ...
    // [4/4] Create V055DemoApp
    Serial.println("[4/4] Creating V055DemoApp...");
    Serial.flush();

    static V055DemoApp demoApp;
    g_demoApp = &demoApp;

    if (!g_demoApp->begin(g_relativeDisplay)) {
        displayError("V055DemoApp initialization failed");
        while (1) delay(1000);
    }

    Serial.println("  [PASS] V055DemoApp initialized");
    Serial.println();
    ...
    Serial.println("  3. V0.5 Demo:");
    Serial.println("     - Logo Animation (wait 2s + animate 1.5s + hold 2s)");
    Serial.println("     - Scientific Mode (5 seconds)");
    Serial.println("     - Compact Mode (5 seconds)");
