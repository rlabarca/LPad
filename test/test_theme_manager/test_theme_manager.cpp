...
/**
 * Test: All semantic color roles are defined (non-zero)
 * Scenario: All semantic color roles are defined
 */
void test_all_semantic_color_roles_defined() {
    const Theme* theme = ThemeManager::getInstance().getTheme();
    TEST_ASSERT_NOT_NULL(theme);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.background);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.surface);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.primary);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.secondary);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.accent);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.text_main);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.text_secondary);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.text_error);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.graph_axes);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.graph_ticks);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.axis_labels);
    TEST_ASSERT_NOT_EQUAL(0, theme->colors.data_labels);
}

/**
 * Test: All 5 font pointers are non-null
 * Scenario: All font pointers are non-null
 */
void test_all_font_pointers_nonnull() {
    const Theme* theme = ThemeManager::getInstance().getTheme();
    TEST_ASSERT_NOT_NULL(theme->fonts.smallest);
    TEST_ASSERT_NOT_NULL(theme->fonts.normal);
    TEST_ASSERT_NOT_NULL(theme->fonts.ui);
    TEST_ASSERT_NOT_NULL(theme->fonts.heading);
    TEST_ASSERT_NOT_NULL(theme->fonts.title);
}

/**
 * Test: Background color is Night (0x1923)
 * Scenario: Background color is Night
 */
void test_background_color_is_night() {
    const Theme* theme = ThemeManager::getInstance().getTheme();
    TEST_ASSERT_EQUAL_UINT16(0x1923, theme->colors.background);
}
...
int main(int argc, char** argv) {
    ...
    RUN_TEST(test_all_semantic_color_roles_defined);
    RUN_TEST(test_all_font_pointers_nonnull);
    RUN_TEST(test_background_color_is_night);
