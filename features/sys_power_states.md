...
### [2026-02-16] CST816 Init Needs Longer Delays and Wire.end() After Deep Sleep Wakeup
**Problem:** After ESP32 deep sleep wakeup (full reboot), `hal_touch_init()` fails with NACK (error 2) on all 3 attempts. The CST816 was in auto-sleep during the entire deep sleep period. Two issues: (1) The 50ms post-INT-release settle time is insufficient — after extended auto-sleep, the CST816 needs up to 300ms to become I2C-responsive. (2) `Wire.begin()` checks an internal `_started` flag and skips reinitialization on calls 2+, so the I2C peripheral retains stale state from the failed probe. **Fix:** Call `Wire.end()` before each attempt to force a clean I2C peripheral init. Increase post-release settle delay progressively (100→300ms). Increase to 5 attempts (from 3). Total worst-case init time ~2.5s, acceptable during boot.
