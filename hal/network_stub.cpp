...
static const char* g_stub_ssid = "Demo WiFi";

bool hal_network_init(const char* ssid, const char* password) {
    // Spec §2.1: return false if both parameters are null
    if (ssid == nullptr || password == nullptr) {
        g_stub_status = HAL_NETWORK_STATUS_ERROR;
        return false;
    }
...
void hal_network_stub_set_ssid(const char* ssid) {
    g_stub_ssid = ssid;
}
#endif
